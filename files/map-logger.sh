...
if [ $# -ne 3 ]
then
  echo "Usage: logJob <job_ID> <log_to_location> <num_mappers>"
  exit 1
fi
...
loop=0
while [ $loop -lt $numMappers ]
do
  formattedCount=`printf "%05d" "$loop"`
  wget -O $saveTo/taskstats$formattedCount.html "http://$jobtracker:50030/taskstats.jsp?tipid=task_"$idNumber"_m_0"$formattedCount
  loop=`expr $loop + 1`
done
